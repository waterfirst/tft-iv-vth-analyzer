import { generateExamples, parseCSV, groupCurves, extractVth, summarize, rowsToCSV } from './core.mjs';
...
function recalculate() {
  ...
  state.results = state.curves.map(curve => {
    ...
    return { curve, ...result, spec: result.ok ? result.vth >= min && result.vth <= max : null };
  });
...
function renderTransfer() {
  ...
  if (state.scale === 'log') layout.yaxis.range = [-13.5, -3.5];
  const selectedResult = state.results.find(r => r.curve.key === state.selected && r.ok);
  if (selectedResult) {
    layout.shapes = [{
...
function renderDistribution() {
  ...
  const visible = state.results.filter(r => r.ok && (state.type === 'both' || r.curve.type.toLowerCase() === state.type));
  const traces = ['NMOS', 'PMOS'].map(type => ({
    x: visible.filter(r => r.curve.type === type).map(r => r.vth),
    type: 'histogram', name: type, opacity: .68,
    marker: { color: palette[type], line: { color: '#ffffff', width: .5 } },
    hovertemplate: `${type}<br>Vth %{x:.3f} V<br>count %{y}<extra></extra>`,
    nbinsx: 18
  })).filter(trace => trace.x.length);
  const layout = plotLayout('Device count');
  layout.xaxis.title = 'Extracted Vth (V)';
...
function exportResults() {
  const headers = ['device_id', 'type', 'vth_v', 'method', 'fit_r2', 'fit_status', 'spec_status', 'point_count', 'source'];
  const lines = [headers.join(','), ...state.results.map(r => [
    r.curve.id, r.curve.type, r.ok ? r.vth.toFixed(8) : '', methodLabel[state.method], Number.isFinite(r.r2) ? r.r2.toFixed(8) : '',
...
function bind() {
  ...
  ['currentTarget', 'normalizeGeometry', 'nmosMin', 'nmosMax', 'pmosMin', 'pmosMax'].forEach(id => $(id).addEventListener('change', recalculate));
